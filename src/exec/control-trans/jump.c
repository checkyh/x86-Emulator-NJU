...
make_helper (j_short)
{
	...
	switch (jump_ins)
	{	case (0xe3):
		if (suffix=='l')   {  sprintf(jump_type,"%s","jecx");if(cpu.ecx==1) ADDR(cpu.eip,short_addr,8) }
		else {sprintf(jump_type,"%s","jcx");if(reg_w(1)==1) ADDR(cpu.eip,short_addr,8)}break;
		case (0xeb):sprintf(jump_type,"%s","jump");ADDR(cpu.eip,short_addr,8) break;//jump 8
		case (0x70):sprintf(jump_type,"%s","jo");if(cpu.OF==1) ADDR(cpu.eip,short_addr,8)  break;
		case (0x71):sprintf(jump_type,"%s","jno");if(cpu.OF==0) ADDR(cpu.eip,short_addr,8)  break;
		case (0x72):sprintf(jump_type,"%s","jnae");if(cpu.CF==1) ADDR(cpu.eip,short_addr,8)  break;
		case (0x73):sprintf(jump_type,"%s","jae");if(cpu.CF==0) ADDR(cpu.eip,short_addr,8)  break;//JAE
		case (0x74):sprintf(jump_type,"%s","je");if(cpu.ZF==1) ADDR(cpu.eip,short_addr,8)  break;//JE/JZ
		case (0x75):sprintf(jump_type,"%s","jne");if(cpu.ZF==0)  ADDR(cpu.eip,short_addr,8) break;//JNE
		case (0x76):sprintf(jump_type,"%s","jbe");if(cpu.ZF==1||cpu.CF==1)  ADDR(cpu.eip,short_addr,8) break;//Jbe
		case (0x77):sprintf(jump_type,"%s","ja");if(cpu.CF==0&&cpu.ZF==0) ADDR(cpu.eip,short_addr,8)  break;//ja
		case (0x78):sprintf(jump_type,"%s","js");if(cpu.SF==1) ADDR(cpu.eip,short_addr,8)  break;
		case (0x79):sprintf(jump_type,"%s","jns");if(cpu.SF==0) ADDR(cpu.eip,short_addr,8)  break;
		case (0x7a):sprintf(jump_type,"%s","jp");if(cpu.PF==1) ADDR(cpu.eip,short_addr,8)  break;
		case (0x7b):sprintf(jump_type,"%s","jpo");if(cpu.PF==0) ADDR(cpu.eip,short_addr,8)  break;
		case (0x7c):sprintf(jump_type,"%s","jl");if(cpu.SF!=cpu.OF) ADDR(cpu.eip,short_addr,8)  break;//Jl
		case (0x7d):sprintf(jump_type,"%s","jge");if(cpu.SF==cpu.OF) ADDR(cpu.eip,short_addr,8)  break;//Jge	 
		case (0x7e):sprintf(jump_type,"%s","jle");if(cpu.ZF==1||cpu.SF!=cpu.OF) ADDR(cpu.eip,short_addr,8)  break;//Jle
		case (0x7f):sprintf(jump_type,"%s","jg");	if(cpu.ZF==0 &&cpu.SF==cpu.OF) ADDR(cpu.eip,short_addr,8)  break;//Jle
		default:break;
	}
