...
make_helper(xd3_switcher)//bin-logcial cl
{
	ModR_M m;
	m.val = instr_fetch(eip + 1, 1);
	binlogical_change(_cl2rm_v)
}
make_helper(x0f_switcher)//2 byte escape
{
	 ...
	 if (ins_type>=0x40&&ins_type<=0x4f) return 1+cmov_v(eip+1);
	 else
	 {

	 switch(ins_type)
	 {
	 	case (0x01):return  1+ldt(eip+1);
	 	case (0x20):return 1+mov_c2r(eip+1);
	 	case (0x22):return 1+mov_r2c(eip+1);
	 	case (0xaf):return 1+imul_rm2r_v(eip+1);
	 	case (0xbe):return 1+movsb_v(eip+1);
	 	case (0xbf):return 1+movsw(eip+1);
