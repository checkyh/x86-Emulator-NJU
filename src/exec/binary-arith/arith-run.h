...
#define switch_r {result=*dst+src; if(result<*dst||result<src) cpu.CF=1;else cpu.CF=0;*dst=*dst+src;\
	if((MSB(src)&&MSB(*dst)&&MSB(result)==0)||(MSB(src)==0&&MSB(*dst)==0&&MSB(result)))cpu.OF=1;else cpu.OF=0;}
...
#define switch_r {if(*dst<src) {result=*dst+(~src)+1;cpu.CF=1;*dst=*dst+~src+1;}else {result=*dst-src;cpu.CF=0;*dst=*dst-src;}\
	if((MSB(src)&&MSB(*dst)==0&&MSB(result))||(MSB(src)==0&&MSB(*dst)&&MSB(result)==0))cpu.OF=1;else cpu.OF=0;}
#define switch_rm {if(*dst<src) {result=*dst+(~src)+1;cpu.CF=1;MEM_W(addr,*dst+(~src)+1);}else {result=*dst-src;cpu.CF=0;MEM_W(addr,*dst-src);}\
	if((MSB(src)&&MSB(*dst)==0&&MSB(result))||(MSB(src)==0&&MSB(*dst)&&MSB(result)==0))cpu.OF=1;else cpu.OF=0;}
