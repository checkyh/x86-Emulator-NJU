...
#define OF_check(result)  if(result<-128UL||result>127UL) {assert(0);cpu.OF=1;}else cpu.OF=0;
...
#define OF_check(result)  if(result<-32768UL||result>32767UL) {assert(0);cpu.OF=1;}else cpu.OF=0;

#elif DATA_BYTE == 4
#define SUFFIX l
#define DATA_TYPE uint32_t
#define DATA_TYPE_S int32_t
#define OF_check(result)  if(result<-2147483648UL||result>2147483647UL) {assert(0);cpu.OF=1;}else cpu.OF=0;
